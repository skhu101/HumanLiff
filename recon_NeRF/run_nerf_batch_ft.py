...
def create_nerf(args, device=None):
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        if not args.ft_triplane_only:
            start = ckpt['global_step']
        
        # Load model
        if args.ft_triplane_only:
            if args.layer_idx == 0:
                triplane = ckpt['network_fn_state_dict']['tri_planes'][0:1].clone()
                ckpt['network_fn_state_dict']['tri_planes'] = triplane
            else:
                human_name = global_args.data_root.split('/')[-1]
                triplane_path = 'logs/' + global_args.expname + f'/{human_name}_002000.tar' 
                triplane = torch.load(triplane_path, map_location='cpu')['network_fn_state_dict']['tri_planes']
                triplane[:, args.layer_idx] = triplane[:, args.layer_idx-1]
                ckpt['network_fn_state_dict']['tri_planes'] = triplane
...
def train(nprocs, global_args):
    ...
    while global_step < n_iters:
        ...
        for i, data in enumerate(training_loader):
            ...
            if global_args.ft_triplane_only:
                decay_rate = 0.5
                decay_steps = 500
                new_lrate = args.tri_plane_lrate * (decay_rate ** (global_step / decay_steps))       
                for param_group in optimizer.param_groups:
            ...
            if global_step % args.i_print == 0 and global_step > 1:
                ...
                if (args.ddp and dist.get_rank() == 0) or not args.ddp:
                    print("[TRAIN] Epoch:{}  Iter: {} Lr: {} Loss: {} Img Loss: {} Acc Loss: {} tv Loss: {} L1 Loss: {} PSNR: {}  Time: {} s/iter".format(epoch, global_step, new_lrate, round(running_loss / args.i_print, 5), round(running_img_loss / args.i_print, 5), round(running_acc_loss / args.i_print, 5), round(running_tv_loss / args.i_print, 5), round(running_l1_loss / args.i_print, 5), psnr, round(dt, 3)))
                running_loss = 0.0
                running_img_loss = 0.0
                running_acc_loss = 0.0
                running_tv_loss = 0.0
            ...
            if global_step % args.i_weights == 0 and global_step > 1:
                human_name = global_args.data_root.split('/')[-1]
                path = os.path.join(basedir, expname, f'{human_name}_{str(global_step).zfill(6)}.tar')

                if (args.ddp and dist.get_rank() == 0) or not args.ddp:
                    triplane_dict = {}
                    triplane_dict['tri_planes'] = render_kwargs_train['renderer'].module.state_dict()['tri_planes'].clone()
                    torch.save({
                        'network_fn_state_dict': triplane_dict,
                    }, path)
...
if __name__=='__main__':
    ...
    test_SynBody_list_all = [os.path.join(data_root_dir, human_name.strip()) for human_name in human_names][global_args.start_idx:global_args.end_idx]

    for id, human_data_path in enumerate(test_SynBody_list_all):
        global_args.data_root = human_data_path
        global_args.human_id = id + global_args.start_idx
        for layer_idx in range(4):
            global_args.layer_idx = layer_idx
