...
class TightCapDatasetBatch(Dataset):
    def __init__(self, data_root=None, split='test', multi_person=True, num_instance=1, pose_start=0, pose_interval=5, poses_num=1, views_num=382, N_rand=4096, image_scaling=1.0, smpl_type='smpl', layer_idx=None):
        super(TightCapDatasetBatch, self).__init__()
        ...
        self.human_list = os.path.join(all_human_data_root, 'TightCap_human_list.txt')
    ...
    def __getitem__(self, index):
        """
            ...
            bkgd_msk_all: mask of foreground and background
                ...
                test: no use
        """
        # instance_idx = index // (self.ni * len(self.output_view)) if self.multi_person else 0
        # pose_index = ( index % (self.ni * len(self.output_view)) ) // len(self.output_view) * self.i_intv + self.i
        ...
        rgb, ray_o, ray_d, near, far, coord_, mask_at_box, bkgd_msk = if_nerf_dutils.sample_ray_batch(
                img, msk, K, R, T, world_bounds, self.N_rand, self.split)
