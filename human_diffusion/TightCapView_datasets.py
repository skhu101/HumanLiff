...
class TightCapViewDataset(Dataset):
    def __init__(self, triplane_root=None, img_root=None, split='test', num_instance=1, pose_start=0, pose_interval=5, pose_num=1, view_num=185, N_rand=2048, image_scaling=1.0, layer_idx=None, smpl_type='smpl', start_motion_pose_id=2000):
        super(TightCapViewDataset, self).__init__()
        ...
        with open(self.human_list) as f:
            for line in f.readlines()[0:num_instance]:
                human_name = line.strip().split('_002000.tar')[0]
                human_dirs.append(human_name)
        ...
        for subject_root in self.root_list:
            camera_file = os.path.join(subject_root, 'person-top-bottom-shoes', 'cameras.json')
    ...
    def __getitem__(self, index):
        ...
        if cloth_layer_index == 0:
            full_img_path = os.path.join(self.data_root, 'person-top-bottom-shoes', 'img', f'camera{str(view_index).zfill(4)}', str(pose_index).zfill(4)+'.jpg')   
            full_mask_path = os.path.join(self.data_root, 'person-top-bottom-shoes', 'mask', f'camera{str(view_index).zfill(4)}', str(pose_index).zfill(4)+'.png') 

            naked_mask_path = os.path.join(self.data_root, 'person', 'mask', f'camera{str(view_index).zfill(4)}', str(pose_index).zfill(4)+'.png')
            top_mask_path = os.path.join(self.data_root, 'top', 'mask', f'camera{str(view_index).zfill(4)}', str(pose_index).zfill(4)+'.png')
            bottom_mask_path = os.path.join(self.data_root, 'bottom', 'mask', f'camera{str(view_index).zfill(4)}', str(pose_index).zfill(4)+'.png')
            shoes_mask_path = os.path.join(self.data_root, 'shoes', 'mask', f'camera{str(view_index).zfill(4)}', str(pose_index).zfill(4)+'.png')

            img = np.array(imageio.imread(full_img_path).astype(np.float32) / 255.)
            full_mask = np.array(self.get_mask(full_mask_path)) / 255.
            naked_mask = np.array(self.get_mask(naked_mask_path)) / 255.
            top_mask = np.array(self.get_mask(top_mask_path)) / 255.
            bottom_mask = np.array(self.get_mask(bottom_mask_path)) / 255.
            shoes_mask = np.array(self.get_mask(shoes_mask_path)) / 255.
            
            img[(top_mask == 1) & ((naked_mask + top_mask) == 1)] = 0
            img[(bottom_mask == 1) & ((naked_mask + bottom_mask) == 1)] = 0
            img[(shoes_mask == 1) & ((naked_mask + shoes_mask) == 1)] = 0
            img[(naked_mask + top_mask + bottom_mask + shoes_mask) >=2] = np.array([0.607186, 0.49289057, 0.43795943])
            img[full_mask == 0] = 0
            msk = np.zeros_like(full_mask)
            msk[img.sum(-1)!=0] = 1   
            ...
            img[(top_mask == 1) & ((naked_mask + top_mask) == 1)] = 0
            img[(shoes_mask == 1) & ((naked_mask + shoes_mask) == 1)] = 0
            img[(naked_mask + top_mask + shoes_mask) >=2] = np.array([0.607186, 0.49289057, 0.43795943])
            img[full_mask == 0] = 0
            msk = np.zeros_like(full_mask)
            msk[img.sum(-1)!=0] = 1 
            ...
            img[(shoes_mask == 1) & ((naked_mask + shoes_mask) == 1)] = 0
            img[(naked_mask + shoes_mask) >=2] = np.array([0.607186, 0.49289057, 0.43795943])
            img[full_mask == 0] = 0
            msk = np.zeros_like(full_mask)
            msk[img.sum(-1)!=0] = 1 
        elif cloth_layer_index == 3:
            img_path = os.path.join(self.data_root, 'person-top-bottom-shoes', 'img', f'camera{str(view_index).zfill(4)}', str(pose_index).zfill(4)+'.jpg')            
            mask_path = os.path.join(self.data_root, 'person-top-bottom-shoes', 'mask', f'camera{str(view_index).zfill(4)}', str(pose_index).zfill(4)+'.png') 
        ...
        world_bounds, vertices, params, joints = self.prepare_input(smpl_path, pose_index)

        # map smpl joints to image space
        RT = torch.cat([torch.tensor(R), torch.tensor(T).reshape(3,1)], -1)[None, None]
        xyz = torch.repeat_interleave(torch.tensor(joints)[None, None], repeats=RT.shape[1], dim=1) #[bs, view_num, , 3]
        xyz = torch.matmul(RT[:, :, None, :, :3].float(), xyz[..., None].float()) + RT[:, :, None, :, 3:].float()
        xyz = torch.matmul(torch.tensor(K)[None, None][:, :, None].float(), xyz)[..., 0]
        xy = xyz[..., :2] / (xyz[..., 2:] + 1e-5)
        joints_uv = xy.view(-1, *xy.shape[2:])
